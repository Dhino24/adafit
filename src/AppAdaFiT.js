...
// Données exercices par groupe musculaire (abrégées)
const exData = {
  poitrine: [
...
const App = () => {
  ...
  // Ouvrir/fermer détails exercice
  ...
  return (
    <div className="flex flex-col h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      {/* Header */}
      <header className={`bg-gradient-to-r ${getTheme().header} text-white p-4 shadow-lg transition-all duration-300`}>
        <div className="flex justify-between items-center">
          <div className="flex items-center space-x-2">
            <Dumbbell className="w-6 h-6" />
            <h1 className="text-xl font-bold">FitTrack Diaspora</h1>
          </div>
          <div className="flex items-center space-x-3">
            <span className={`bg-gradient-to-r ${getTheme().accent} px-3 py-1 rounded-full text-sm font-medium`}>Semaine 3/12</span>
            <span className="bg-gradient-to-r from-red-500 to-pink-500 px-3 py-1 rounded-full text-sm font-medium">Objectif: 100kg</span>
          </div>
        </div>
        <div className={`mt-4 flex items-center bg-gradient-to-r ${getTheme().headerLight} p-2 rounded-lg backdrop-blur-sm`}>
          <div className="flex-1">
            <div className="text-xs uppercase font-medium">Poids actuel</div>
            <div className="text-lg font-bold flex items-center">
              <ArrowUp className="w-4 h-4 mr-1 text-green-300" />93kg
            </div>
          </div>
          <div className="flex-1">
            <div className="text-xs uppercase font-medium">Progression développé</div>
            <div className="text-lg font-bold flex items-center">
              <ArrowUp className="w-4 h-4 mr-1 text-green-300" />60kg → 65kg (+5kg)
            </div>
          </div>
          <div className="flex-1">
            <div className="text-xs uppercase font-medium">Progression squat</div>
            <div className="text-lg font-bold flex items-center">
              <ArrowUp className="w-4 h-4 mr-1 text-green-300" />50kg → 60kg (+10kg)
            </div>
          </div>
        </div>
      </header>
      ...
      {/* Barre recherche */}
      <div className="px-4 py-2 bg-white shadow-md sticky top-0 z-10">
        <div className="relative max-w-xl mx-auto">
          <Search size={18} className="absolute left-3 top-3 text-indigo-400" />
          <input 
            type="text" 
            placeholder="Rechercher un exercice..." 
            className="w-full p-2 pl-10 border border-indigo-200 rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
            value={searchQ}
      ...
      {searchQ && filteredEx.length > 0 && (
        <div className="p-4 bg-white shadow-md max-h-64 overflow-y-auto">
          <h3 className="text-sm font-semibold mb-2 text-indigo-600">Résultats de recherche</h3>
          <div className="grid grid-cols-1 gap-2">
            {filteredEx.map(ex => (
              <div 
                key={ex.id} 
                className="p-3 rounded-lg border border-indigo-100 hover:bg-indigo-50 cursor-pointer"
      ...
      <main className="flex-1 overflow-y-auto px-4 pb-4">
        {/* Onglets principaux (fixés) */}
        <div className="sticky top-0 z-30 pt-4 pb-2 bg-gray-50 bg-opacity-95 backdrop-blur-sm">
          <div className="flex justify-between bg-white rounded-xl p-1 shadow-md max-w-6xl mx-auto">
            {['program', 'nutrition', 'progress', 'exercises'].map(tab => (
              <div 
                key={tab}
                className={`flex-1 py-3 px-1 text-center rounded-lg cursor-pointer transition ${activeTab === tab ? 
              ...
              >
                {tab === 'program' ? <Calendar size={18} className="inline mr-2" /> :
                 tab === 'nutrition' ? <Coffee size={18} className="inline mr-2" /> :
                 tab === 'progress' ? <BarChart2 size={18} className="inline mr-2" /> :
              ...
              <div className="flex space-x-2 overflow-x-auto py-2">
                {Object.entries(workoutProg).map(([day, data]) => (
                  <div 
                    ...
                    className={`whitespace-nowrap px-3 py-2 text-sm rounded-lg cursor-pointer flex items-center transition ${curDay === day ? 
                      'bg-gradient-to-r ' + getTheme().primary + ' text-white font-medium shadow-sm' : 
                      'bg-white text-gray-700 shadow-sm hover:bg-indigo-50'}`}
                  >
                    ...
                    <div key={idx}>
                      ...
                      <div className="space-y-4">
                        {mg.exercises.map((ex, exIdx) => (
                          <div key={exIdx} className="bg-white rounded-xl border border-indigo-100 shadow-sm hover:shadow-md p-4">
                            <div className="flex justify-between items-start">
                              <div className="flex-1">
                                <h4 className="font-semibold text-indigo-800 text-lg mb-1">{ex.name}</h4>
                                <p className="text-sm text-indigo-500 mb-2">{ex.sets} séries × {ex.reps} reps • repos {ex.rest}</p>
                                {ex.goal && ex.current && (
                                  <div className="mt-3">
                              ...
                              <div className="flex flex-col space-y-2 ml-4">
                                <button 
                                  className={`bg-gradient-to-r ${getTheme().action} hover:${getTheme().actionHover} text-white px-4 py-2 rounded-lg shadow-sm hover:shadow-md transition flex items-center text-sm`}
                                  onClick={() => startTimer(ex)}
                                >
                                  <Clock size={14} className="mr-2" />Démarrer
                                </button>
                                <button 
                                  className={`bg-gradient-to-r ${getTheme().button} hover:${getTheme().buttonHover} text-white px-4 py-2 rounded-lg shadow-sm hover:shadow-md transition flex items-center text-sm`}
                                  onClick={() => {
                                    const fullEx = Object.values(exData).flat().find(e => e.name === ex.name);
                                    openExDetail(fullEx || ex);
                                  }}
                                >
                                  <ChevronRight size={14} className="mr-2" />Détails
                                </button>
                              </div>
                            </div>
                          </div>
      ...
      <footer className={`bg-gradient-to-r ${getTheme().header} text-white p-3 shadow-inner transition-all duration-300`}>
        <div className="flex justify-between items-center max-w-6xl mx-auto">
          <div className="flex items-center">
            ...
            <span className="text-sm font-medium">{APP.name} v{APP.version}</span>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-indigo-900 bg-opacity-95 backdrop-blur-lg">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden">
            <div className="bg-indigo-600 text-white p-5">
              ...
              <div className="text-sm mt-2 bg-white/20 px-3 py-2 rounded-lg">
